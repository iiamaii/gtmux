...
pub async fn allowlist_post_handler(
...
pub async fn open_handler(State(state): State<AppState>, req: Request<Body>) -> Response {
    ...
    match spawn::spawn(&canonical) {
        Ok(()) => {
            if matched {
                state.file_open.audit.record_allowed(&canonical_str, &cookie_prefix);
            } else {
                state.file_open.audit.record_one_time(&canonical_str, &cookie_prefix);
            }
...
mod tests {
    ...
    fn test_state_with_file_open() -> (crate::AppState, TokenString, TempDir) {
        ...
        state.file_open = FileOpenContext::for_tests(
