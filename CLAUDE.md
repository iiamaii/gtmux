...
- README, CLAUDE.md, and other repo-meta files: English (this file is the example).
...
**ADR ↔ plan/handover coherence is a hard rule**: when amending or accepting an ADR, *also* update every linked forward-looking plan/handover/report doc (`docs/plans/*.md`, `docs/reports/*-handover*.md`, `docs/reports/*-next-batch*.md`, work-package docs, queue docs). Before finishing an ADR amend, search the repo for references to the changed decision and bring them in sync — leaving plan-side docs out of date creates silent contradictions that future agents cold-pickup as truth. Same applies in reverse: when re-prioritising a plan, surface the rationale into the relevant ADR if it changes a documented decision.
