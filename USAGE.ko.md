...
경우 (ADR-0026 server-id mismatch → 강제 re-attach).
...
`Export` 는 `docs/ssot/canvas-layout-schema.md` 의 단일 `.json` 을
쓴다:
...
이전엔 충분. 전체 workspace archive 는 아니다 (scope 외, ADR-0029).
...
```
 ...
 │  · http-api crate      │  --port 1쌍당 1프로세스 (1:1:1, ADR-0007).
 ...
 └────────────┬───────────┘
              ...
              │
 ┌────────────▼───────────┐
 │  Terminal server       │  pty-backend crate (ADR-0013/0014).
...
- HTTP API: `GET/PUT /api/layout`, `GET /api/sessions`,
  ...
  (`GET /api/files/stat`, ADR-0034), auth
  (`POST /auth/login`, `POST /auth/logout`).
- WebSocket: 양방향 control + data 프레임 (`docs/ssot/wire-protocol.md`
  참조). Terminal 별 출력과 notification 프레임을 multiplex.
...
PTY supervisor 는 gtmux 프로세스 **안에** 있다 — 별도 daemon 을 띄울
필요가 없다. 런타임에 tmux 를 쓰지 않는다 (ADR-0013); 프로젝트
이름은 MVP 전에 폐기된 tmux control mode 시절 설계의 잔재.
...
- FE 로컬 clipboard (page lifetime), undo/redo stack (50 entries,
  in-memory).
- 커스텀 keyboard shortcut override → `localStorage` (ADR-0017 D6).
...
- **서버 재기동 (`gtmux stop` + `start`)**: terminal 은 **소멸**
  (ADR-0014 D5). Layout JSON 은 재연결 시 replay 되지만 모든 panel 이
...
- Auto-mount cascade (ADR-0015): 연속 spawn 시 FE 가 ~40px 씩 offset
  주어 완전 겹치지 않게.
...
- Document viewer 를 drag-spawn. 두 가지 content 모드:
  ...
  - Asset-backed: 파일 업로드 → 서버가
    `<workspace>/.assets/<sha256>` 에 저장 (ADR-0033), item 은
    `asset_id` 보유.
- 3가지 render 모드를 노드 위 버튼으로 순환:
  - **Rendered**: Markdown → HTML, DOMPurify 로 sanitize.
  - **Interactive**: sandboxed iframe (ADR-0037) — 전체 HTML/JS 허용
    되지만 origin 격리.
  - **Source**: raw text.
...
- Picker 는 허용된 root 만 traverse (ADR-0035 — 기본 home directory,
  설정 가능).
- Server-side `GET /api/files/stat` 가 메타데이터만 반환 (ADR-0034).
...
(ADR-0017 D6). Override 는 `localStorage` 에 저장, 브라우저 scope.
기본 binding:
...
**Group** 은 web-only 계층 컨테이너 — Figma 의 frame 에 해당하며,
tmux Window 의 자리를 대신한다. Label, color, visibility, lock, 자식
의 순서 list 를 보유. Group 은 자체 geometry 를 **들고 있지 않다**
(G-hybrid 모델, ADR-0010): 그룹의 시각 bound 는 자식들로부터 매
render 마다 derive.
...
Z-index 와 tree 는 **독립** (ADR-0024). Layer tree row 순서 변경은
...
- Alignment: left / center / right / top / middle / bottom + Distribute
  H / Distribute V — Inspector (`ItemInfoView`, ADR-0027) 와 context
  menu 의 버튼.
...
- [`docs/sketch.md`](docs/sketch.md) — 전체 설계 spec (한국어), scope
  의 source of truth.
- [`docs/adr/`](docs/adr/) — accepted 아키텍처 결정 (특히 0007 서버
  binding, 0008 single-pane 컨벤션, 0010 Group 모델, 0013/0014 PTY
  backend, 0017 layout, 0018 canvas item data model, 0019 session
  모델, 0020 auth lifecycle, 0021 terminal pool/mirror, 0024 z-index,
  0027 inspector, 0028 undo/redo, 0029 import/export, 0030 clipboard).
