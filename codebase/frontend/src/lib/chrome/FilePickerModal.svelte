<script lang="ts">
  ...
  interface Props {
    ...
    initialDir?: string;
    /** Optional file allowlist. Directories remain visible for navigation. */
    accept?: { extensions: string[]; description: string } | null;
  }

  const { open, onCancel, onSelect, onUnauthorized, initialDir = '', accept = null }: Props = $props();
  ...
  const acceptExts = $derived(accept?.extensions.map((ext) => ext.toLowerCase()) ?? []);

  function entryMatchesAccept(entry: FsEntry): boolean {
    if (accept === null || entry.kind === 'directory') return true;
    const lower = entry.name.toLowerCase();
    return acceptExts.some((ext) => lower.endsWith(ext));
  }

  const filteredEntries = $derived.by((): FsEntry[] => {
    const accepted = entries.filter(entryMatchesAccept);
    if (filter.length === 0) return accepted;
    const needle = filter.toLowerCase();
    return accepted.filter((e) => e.name.toLowerCase().includes(needle));
  });

  const selectedEntry = $derived.by((): FsEntry | null => {
    if (selectedName === null) return null;
    return entries.find((e) => e.name === selectedName) ?? null;
  });
  ...
  const canSelect = $derived(
    selectedAbsolute !== null
      && (accept === null || (selectedEntry?.kind === 'file' && entryMatchesAccept(selectedEntry))),
  );
  ...
  function onEntryDblClick(entry: FsEntry): void {
    if (entry.kind === 'directory') {
      ...
      if (!entryMatchesAccept(entry)) return;
      // file → select + close
  ...
  function onSelectClick(): void {
    if (!canSelect || selectedAbsolute === null) return;
  ...
  {#snippet body()}
    ...
    <div class="picker-filter">
      <input
        ...
        placeholder={accept === null ? 'Filter…' : `Filter ${accept.description.toLowerCase()}…`}
        value={filter}
      ...
      {:else if filteredEntries.length === 0}
        <p class="state">
          {filter.length > 0
            ...
            : accept === null
              ? 'Empty directory.'
              : `No ${accept.description.toLowerCase()} files.`}
        </p>
      {:else}
    ...
    <div class="picker-selected mono" title={selectedAbsolute ?? ''}>
      {selectedAbsolute ?? (accept === null ? '— Select a file —' : `— Select ${accept.description.toLowerCase()} —`)}
    </div>
  {/snippet}
  {#snippet footer()}
