 ...
 * 256 KiB. **BE 의 ring buffer cap = 128 KiB** per PaneId (`pty-backend
 * /src/lib.rs::RING_CAPACITY`). 본 cap 이 BE ring 의 *최대 catch-up
 * replay snapshot* (128 KiB) 을 2× 안전 margin 으로 cover — 충분 size.
 *
 * 0077 follow-up: 옛 추정에서 본 cap 을 4 MiB 로 잠시 올렸으나 BE
 * `RING_CAPACITY` 확인 후 *원래 256 KiB 가 정합* 임을 확정 → revert.
 * History 손실의 root cause 는 cap 이 아니라 svelte-flow 의
 * `onlyRenderVisibleElements` virtualization 으로 PanelNode 가 unmount
 * 되어 xterm scrollback 자체 lost 되는 path — `Canvas.svelte` 의 fix
 * 가 root.
 */
...
function handleMountCascade(payload: Uint8Array): void {
  ...
  // ADR-0021 D3 amend ② (0072 BE follow-up §1): session-switch race guard.
  // BE filters per-frame by `hub.session_for_owner(owner) == event.trigger_session`,
  // but a frame in flight when the owner switches sessions would otherwise
  // land on the wrong session. The wire payload's `trigger_session` lets
  // us drop the cascade if our state has already flipped.
  if (decoded.triggerSession !== name) {
    console.warn(
      `[ws] MOUNT_CASCADE for "${decoded.triggerSession}" but active is "${name}" — drop (session-switch race)`,
    );
    return;
  }
